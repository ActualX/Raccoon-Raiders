    ...
    def adjacent_bin_score(self) -> int:
        ...
        for i in range(self.height):
            for j in range(self.width):
                if len(self.at(j, i)) == 1 and \
                        isinstance(self.at(j, i)[0], RecyclingBin):
                    lst.append(self._helper_explore(j, i)) \
                        if self._helper_explore(j, i) != 0 else lst.append(1)
        return max(lst) if lst else 0

    def _helper_explore(self, x: int, y: int, seen: list = None) -> int:
        ...
        acc = len(n)
        for neighbour in n:
            acc += self._helper_explore(neighbour[0], neighbour[1], seen)
        return acc
