...
class GameBoard:
    ...
    _board:
        the overall game board of one game
    ...
    def place_character(self, c: Character) -> None:
        ...
        if isinstance(c, Player):
            ...
            item = self.at(c.x, c.y)[0]
            if isinstance(item, GarbageCan):
                if not item.locked and isinstance(c, Raccoon):
                    c.inside_can = True
    ...
    def to_grid(self) -> List[List[chr]]:
        ...
        for i in range(self.height):
            ...
            for j in range(self.width):
                position = self.at(j, i)
                if len(position) == 0:
                    x_int.append('-')
                elif len(position) == 1:
                    x_int.append(position[0].get_char())
                else:
                    x_int.append('@')
            output.append(x_int)
        return output
    ...
    def check_game_end(self) -> Optional[int]:
        ...
        for i in self._board:
            if isinstance(i, Raccoon):
                if i.inside_can:
                    r_in_can += 1
                elif i.check_trapped():
                    r_trapped += 1
                total_r += 1
        if total_r == r_trapped + r_in_can:
            self.ended = True
            return r_trapped * 10 + self.adjacent_bin_score()
        else:
            self.ended = False
            return None

    def _bin_cluster(self, x: int, y: int, been: list = None) -> int:
        """
        Helper Function.

        In the given position (x, y), find the area of this cluster of bins in
        the grid, and returns the number of bins in this cluster.
        Parameter been is a memory of which bin has already been counted, and
        is ONLY used by recursion step.
        *REMINDER*: the output integer does not include the bin detected by
        adjacent_bin_score().
        ...
        0
        """
        next_directions = get_neighbours((x, y))
        next_direction = next_directions.copy()
        if been is None:
            been = []
        for direction in next_direction:
            if not self.on_board(direction[0], direction[1]) or \
                    len(self.at(direction[0], direction[1])) != 1:
                next_directions.remove(direction)
            else:
                position = self.at(direction[0], direction[1])[0]
                if not isinstance(position, RecyclingBin):
                    next_directions.remove(direction)
                elif (position.x, position.y) in been:
                    next_directions.remove(direction)
        been.extend(next_directions)
        if not next_directions:
            return 0
        acc = len(next_directions)
        for next_step in next_directions:
            acc += self._bin_cluster(next_step[0], next_step[1], been)
        return acc
    ...
    def adjacent_bin_score(self) -> int:
        ...
        for y in range(self.height):
            for x in range(self.width):
                cur_acc = 0
                if len(self.at(x, y)) == 1 and \
                        isinstance(self.at(x, y)[0], RecyclingBin):
                    cur_acc = self._bin_cluster(x, y)
                if cur_acc != 0:
                    acc.append(self._bin_cluster(x, y))
                elif len(self.at(x, y)) == 1 and \
                        isinstance(self.at(x, y)[0], RecyclingBin):
                    acc.append(1)
        if acc:
            return max(acc)
...
class RecyclingBin(Character):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        if self.board.on_board(next_x, next_y):
            next_tile = self.board.at(next_x, next_y)
            if len(next_tile) == 0:
                self.x = next_x
                self.y = next_y
                return True
            next_position = next_tile[0]
            if isinstance(next_position, RecyclingBin):
                if next_position.move(direction):
                    self.x = next_x
                    self.y = next_y
                    return True
        return False
        ...
        if self.board.on_board(next_x, next_y):
            ...
            if isinstance(next_position, RecyclingBin):
                if next_position.move(direction):
                    ...
                    self.y = next_y
                    return True
            elif isinstance(next_position, GarbageCan):
                if next_position.locked:
                    return False
                else:
                    next_position.locked = True
                    return True
        return False
...
class Raccoon(TurnTaker):
    ...
    def check_trapped(self) -> bool:
        ...
        if self.inside_can:
            return False
        next_position = get_neighbours((self.x, self.y))
        for next_x, next_y in next_position:
            if self.board.on_board(next_x, next_y) \
                    and len(self.board.at(next_x, next_y)) == 0:
                return False
            elif len(self.board.at(next_x, next_y)) == 1 and \
                    isinstance(self.board.at(next_x, next_y)[0], GarbageCan):
                return False
        return True
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        if not self.board.on_board(next_x, next_y):
            return False
        position = self.board.at(next_x, next_y)
        if len(position) == 0:
            self.x = next_x
            self.y = next_y
            return True
            ...
            cur_obj = position[0]
            if not isinstance(cur_obj, GarbageCan):
                return False
            else:
                if cur_obj.locked:
                    cur_obj.locked = False
                    return True
                else:
    ...
    def take_turn(self) -> None:
        ...
        for direction in DIRECTIONS:
            ...
            while self.board.on_board(next_x, next_y):
                position = self.board.at(next_x, next_y)
                if len(position) == 0 or \
                        len(position) == 1 and isinstance(position[0], Player):
                    acc += 1
                    next_x += direction[0]
                    next_y += direction[1]
                elif len(position) == 1 and \
                        isinstance(position[0], GarbageCan):
                    next_dir.append(direction)
                    next_step.append(acc)
                    break
                else:
                    break
        if not next_dir and not next_step:
            Raccoon.take_turn(self)
            return None
        else:
            least_step = min(next_step)
            for i in range(len(next_step)):
                if next_step[i] == least_step:
                    self.move(next_dir[i])
                    break
